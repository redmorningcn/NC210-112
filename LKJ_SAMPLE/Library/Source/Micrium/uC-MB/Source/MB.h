...
typedef  struct  modbus_ch {
    ...
    CPU_INT16U       RxFrameHead;                      /* 数据帧头，用于带帧头的非MODSBUS通信                              */
    CPU_INT16U       RxFrameTail;                      /* 数据帧尾，用于带帧尾的非MODSBUS通信                              */
    ...
    CPU_INT08U       AesEn                      : 1;   // AES加密
    CPU_INT08U       NonModbusEn                : 1;   // 非MODBUS通信支持
    CPU_INT08U       IapModbusEn                : 1;   // IAP通信支持
    CPU_INT08U       Retrys                     : 5;   // 重发次数
    OS_SEM          *RxSem;                            // 无名沈增加：指向当前接收信号量
    OS_SEM          *TxSem;                            // 无名沈增加：指向当前发送信号量
    ...
    CPU_INT08U       WrSynNbr;                            // 同步序列号
    CPU_INT08U       RdSynNbr;                            // 同步序列号
    
//#if MODBUS_CFG_SLAVE_EN == DEF_ENABLED    
//    // 寄存器数据类型
//    CPU_INT16U      *pReg;                              // 寄存器地址指针
//#endif
...
MB_EXT   OS_SEM          MB_OS_RxSemTbl[MODBUS_CFG_MAX_CH];// 无名沈修改：从静态变改成全局变量
MB_EXT   OS_SEM          MB_OS_TxSemTbl[MODBUS_CFG_MAX_CH];// 无名沈增加：从静态变改成全局变量
#else
MB_EXT   OS_SEM         *MB_OS_RxSemTbl[MODBUS_CFG_MAX_CH];// 无名沈修改：从静态变改成全局变量
MB_EXT   OS_SEM         *MB_OS_TxSemTbl[MODBUS_CFG_MAX_CH];// 无名沈增加：从静态变改成全局变量
#endif
