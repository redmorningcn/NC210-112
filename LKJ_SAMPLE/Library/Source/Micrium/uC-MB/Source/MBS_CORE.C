...
*/
// FILE MODIFIED BY SE
// added code for SE specific functions and features
...
#define  MBS_FC100_DATA_BYTES    (5u)
...
#if     MODBUS_FC43_EN > 0
// SE added
static   CPU_BOOLEAN          MBS_FC43_readDeviceID         (MODBUS_CH  *pch);
#endif

#if     MODBUS_FC100_EN > 0
//SE added
static   CPU_BOOLEAN          MBS_FC100_scatteredRead       (MODBUS_CH  *pch);
...
{
    pch->StatExceptCtr++;
    //COM_incDiaCtr(COM_EXCEPTIONS);
    // TODO Frame address was new from Micrium
...
CPU_BOOLEAN  MBS_FCxx_Handler (MODBUS_CH  *pch)
{
    ...
    if ((MBS_RX_FRAME_ADDR == pch->NodeAddr) ||       /* Proper node address? (i.e. Is this message for us?)   */
        ...
        (MBS_RX_FRAME_ADDR == MB_getPseudo( ) ) ) {
        pch->StatSlaveMsgCtr++;
        //COM_incDiaCtr(COM_PACKETSTOUNIT);
        ...
        switch (MBS_RX_FRAME_FC) {                           /* Handle the function requested in the frame.           */
           ...
           case MODBUS_FC43_READ_DEV_ID:
              send_reply  = MBS_FC43_readDeviceID(pch);
            break;
#endif

#if MODBUS_FC100_EN > 0 // Scattered Read
            case MODBUS_FC100_SCATTERED_READ:
               send_reply = MBS_FC100_scatteredRead(pch);
            break;
#endif
            ...
            default:                                         /* Function code not implemented, set error response. */
                 ...
                 //COM_incDiaCtr(COM_ILLEGALOPCODE);
                 send_reply = DEF_TRUE;
                 break;
        }
    }
    ...
    {
       //COM_incDiaCtr( COM_PACKETSTOOTHERS );
    }

    if (MBS_RX_FRAME_ADDR == 0 ||
        (MBS_RX_FRAME_ADDR == MB_getPseudo ( ) ) ) {     /* Was the command received a 'broadcast'?   */
...
static  CPU_BOOLEAN  MBS_FC03_HoldingRegRd (MODBUS_CH  *pch)
{
    ...
    // will not get this error, responds with NA value if register does not exist
...
*                                      READ DEVICE ID
*
* Description : Obtains the contents of the devide ID registers.
* Argument(s) : pch       Is a pointer to the Modbus channel's data structure.
*
...
/* Nbr of data bytes must be 3. */
    if (pch->RxFrameNDataBytes != 3)
    {
        return (DEF_FALSE);
    }

    return(DEF_FALSE);// TODO REMOVE THIS ONCE CODE IS IMPLEMENTED
// SE TODO implement device ID code
    ...
    //1st 4 byte are same as request

    // build packect based on id category supported
}
#endif
#endif
...
*                                      SCATTERED READ
*
* Description : Obtains the contents of various registers.
* Argument(s) : pch       Is a pointer to the Modbus channel's data structure.
*
* Return(s)   : TRUE      If a response needs to be sent
*               FALSE     If not
...
/* Nbr of data bytes must be Query data length + 5. */
    if (pch->RxFrameNDataBytes != pch->RxFrameData[2] + MBS_FC100_DATA_BYTES )
    {
        return (DEF_FALSE);
    }
