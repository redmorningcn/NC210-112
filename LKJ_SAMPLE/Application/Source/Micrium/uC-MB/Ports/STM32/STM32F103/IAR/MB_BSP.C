...
void            USARTx_RxTxISRHandler   (USART_TypeDef* USARTx);
                      ...
                      CPU_INT08U  stops)
{
    ...
    */
    ...
    case 0:
        ...
        * 描述： 接收发送使能信号
        */
#define MBR_GPIO_PIN        GPIO_Pin_8             /* PA.08 */
#define MBR_GPIO_PORT       GPIOA
#define MBR_GPIO_RCC        RCC_APB2Periph_GPIOA
        
        GPIO_InitTypeDef  GPIO_InitStructure;
        RCC_APB2PeriphClockCmd(MBR_GPIO_RCC, ENABLE);
        GPIO_InitStructure.GPIO_Pin   = MBR_GPIO_PIN;
        GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
        GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_Out_PP;
        GPIO_Init(MBR_GPIO_PORT, &GPIO_InitStructure);
        
        GPIO_ResetBits(MBR_GPIO_PORT, MBR_GPIO_PIN);        // 低电平接收使能 
        //GPIO_SetBits(MBR_GPIO_PORT, MBR_GPIO_PIN);        // 高电平发送使能        
...
void  USARTx_RxTxISRHandler (USART_TypeDef* USARTx)
{
    ...
    * 描述：获取串口号
    */
    if      ( USARTx == USART1 ) port_nbr = 0;
    else if ( USARTx == USART2 ) port_nbr = 1;
    else if ( USARTx == USART3 ) port_nbr = 2;
...
void  MB_RTU_TmrInit (void)
{
    ...
    * 描述：设置中断入口函数及中断优先级
    */
    BSP_IntVectSet(BSP_INT_ID_TIM2, MB_RTU_TmrISR_Handler);
...
void  MB_RTU_TmrISR_Handler (void)
{
    if(TIM_GetITStatus(TIM2, TIM_IT_Update)== SET) {//检测是否发生溢出更新事件
      TIM_ClearITPendingBit(TIM2 , TIM_FLAG_Update);/* Clear timer #1 interrupt                           */
